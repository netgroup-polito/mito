package it.polito.mito.allocation;
...
import it.polito.mito.functions.EndHost;
import it.polito.mito.functions.Forwarder;
import it.polito.mito.functions.GenericFunction;
import it.polito.mito.functions.LoadBalancer;
import it.polito.mito.functions.NAT;
import it.polito.mito.functions.PacketFilter;
import it.polito.mito.functions.StatefulPacketFilter;
import it.polito.mito.functions.TrafficMonitor;
...
public class AllocationManager {
	 ...
	 */
	 ...
	 /**
	  * This method is invoked by MitoProxy to instantiate the Network Functions which have been specified in the input, processing the input nodes with a proper Configuration (i.e. node which isn't empty). 
		...
		nodes.values().forEach(allocationNode -> {
			...
			if(node.getFunctionalType() != null) {
				if(node.getFunctionalType() == FunctionalTypes.WEBCLIENT) {
					...
					if(node.getConfiguration().get(0).getStatefulFirewall().getDefaultAction() != null) { 
						if(node.getConfiguration().get(0).getStatefulFirewall().getDefaultAction().equals(ActionTypes.ALLOW)) {
							spf.setDefaultAction(true);
						}else if(node.getConfiguration().get(0).getStatefulFirewall().getDefaultAction().equals(ActionTypes.DENY)){
	...
	/**
	 * This method is invoked in the recursive visit of the graph inside Mito Proxy. 
	...
	/**
	 * This method is invoked in MitoProxy before the creation of the Checker but after the recursive visit of the graph. 
