    ...
    public void configureEndHost() {
    	for(Map<Integer, Integer> flowMap : source.getMapFlowIdAtomicPredicatesInInput().values()) {
    		for(Integer traffic : flowMap.values()) {
    			//constraints.add(ctx.mkEq(nctx.deny.apply(source.getZ3Name(), nctx.fpMap.get("bot"), ctx.mkInt(traffic)), ctx.mkFalse()));
    		}
    	}
    	//constraints.add(ctx.mkEq(nctx.deny.apply(source.getZ3Name(), nctx.fpMap.get("bot"), ctx.mkInt(-1)), ctx.mkFalse()));
    }
