...
public class TestCaseGeneratorMitoStates {
	...
	public TestCaseGeneratorMitoStates(String name, int numberStates, int seed) {
		this.name = name;
	...
	public NFV generateNFV(int numberStates, Random rand) {
		...
		int extraFw = numberStates - 5;
		int extraFwA = numberStates / 2;
		int extraFwB = numberStates / 2;
		if (extraFw % 2 != 0) {
			extraFwA += 1; 
		}
		...
		for(int k=0; k<numberNetworks; k++){
			...
			List<Node> firewallsA = new ArrayList<>();
			for(int i=0; i<extraFwA; i++){
				Node fwTMP = createFirewall();
				firewallsA.add(fwTMP);
			}
			List<Node> firewallsB = new ArrayList<>();
			for(int i=0; i<extraFwB; i++){
				Node fwTMP = createFirewall();
				firewallsB.add(fwTMP);
			}
			...
			graph.getNode().addAll(firewallsA);
			graph.getNode().addAll(firewallsB);
			...
			for(int i=0; i<2; i++) {
				for(int j=0; j<2; j++) {
					...
					for(Node n: firewallsA) addFirewallRule(n, 0, ActionTypes.ALLOW, ip1, ip2);
				}				
			}
			for(int i=0; i<2; i++) {
				for(int j=0; j<3; j++) {
					...
					for(Node n: firewallsB) addFirewallRule(n, 0, ActionTypes.ALLOW, ip1, ip2);
				}				
			}
			for(int i=0; i<5; i++) {
