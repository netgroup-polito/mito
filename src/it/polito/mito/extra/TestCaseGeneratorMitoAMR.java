...
public class TestCaseGeneratorMitoAMR {
	...
	public TestCaseGeneratorMitoAMR(String name, int numberAMR, int seed) {
		this.name = name;
		...
		nfv = generateNFV(numberAMR, rand);
	}
	
	
	
	public NFV changeIP(int numberAMR, int seed) {
		this.rand = new Random(seed);
		
		//init

		return generateNFV(numberAMR, rand);
	}
	...
	private void createPolicy(PName type, NFV nfv, Graph graph,
	                          ...
	                          ImpactBias bias, String sPort, Long seed) {
	...
	private void createPolicy(PName type, NFV nfv, Graph graph,
	                          String IPClient, String IPServer,
	                          ImpactBias bias, String sPort) {
	    createPolicy(type, nfv, graph, IPClient, IPServer, bias, sPort, null);
	}

	private void addFirewallRule(Node firewall, int configIndex,
            ActionTypes action, String src, String dst, String sPort) {
			Elements element = new Elements();
			element.setAction(action);
			element.setSource(src);
			element.setDestination(dst);
			element.setSrcPort(sPort);
			element.setDstPort("*");
	...
	public NFV generateNFV(int numberAMR, Random rand) {
		...
		for(int k=0; k<numberNetworks; k++){
			...
			String[] sPorts = new String[501];
			sPorts[0] = "*";
			for (int i = 1; i <= 500; i++) {
			    sPorts[i] = String.valueOf(i);
			}
			
			while(nA < numberAMR) {
				
				String sPort = sPorts[iterationWhile];
				//System.out.println(sPort);
				
				for(int i=0; i<2 && nA < numberAMR; i++) {
					for(int j=0; j<2 && nA < numberAMR; j++) {
						String ip1 = EndPointsG11.get(i).getName();
						String ip2 =EndPointsG12.get(j).getName();
						createPolicy(PName.ATTACK_MITIGATION_PROPERTY, nfv, graph, ip1, ip2, ImpactBias.HIGH, sPort);
						addFirewallRule(firewall1, 0, ActionTypes.ALLOW, ip1, ip2, sPort);
						addFirewallRule(firewall2, 0, ActionTypes.ALLOW, ip1, ip2, sPort);
						nA++;
					}				
				}
				for(int i=0; i<2 && nA < numberAMR; i++) {
					for(int j=0; j<2 && nA < numberAMR; j++) {
						...
						createPolicy(PName.ATTACK_MITIGATION_PROPERTY, nfv, graph, ip1, ip2, ImpactBias.MEDIUM, sPort);
						addFirewallRule(firewall4, 0, ActionTypes.ALLOW, ip1, ip2, sPort);
						addFirewallRule(firewall5, 0, ActionTypes.ALLOW, ip1, ip2, sPort);
						nA++;
					}				
				}
				for(int i=0; i<5 && nA < numberAMR; i++) {
						String ip1 = EndPointsG11.get(2).getName();
						String ip2 =EndPointsG3.get(i).getName();
						createPolicy(PName.ATTACK_MITIGATION_PROPERTY, nfv, graph, ip1, ip2, ImpactBias.LOW, sPort);
						addFirewallRule(firewall1, 0, ActionTypes.ALLOW, ip1, ip2, sPort);
						addFirewallRule(firewall3, 0, ActionTypes.ALLOW, ip1, ip2, sPort);	
						nA++;
				}
				for(int i=0; i<5 && nA < numberAMR; i++) {
