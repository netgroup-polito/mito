...
import it.polito.mito.MitoNormalizer;
...
/**
 * This class implements a parser for Mito output (the z3 model), in order to
...
public class Translator {
	...
	/**
	 * Set the firewall auto-configurated rules in the XML according to the Mito
	 * output
	 */
	public void setPacketFilterAutoConfig() {
		List<Node> autoNodes = originalNfv.getGraphs().getGraph().stream().filter(graph -> graph.getId() == g.getId())
				...
				.filter(n -> n.getFunctionalType() != null && n.getFunctionalType().equals(FunctionalTypes.FIREWALL)
						&& n.getConfiguration().get(0).getFirewall().getElements().isEmpty())
				.collect(Collectors.toList());
		...
		autoNodes.forEach(n -> {
			...
			n.getConfiguration().get(0).getFirewall().setDefaultAction(da);
			...
			n.getConfiguration().get(0).getFirewall().getElements().addAll(listOfRules);
		});
	...
	/**
	 * Remove not used optional network objects from the XML for the Mito output
	 */
	public void setAutoPlacement() {
		...
		usableNodes.forEach(allocationNode -> {
			...
			while (matcher.find()) {
				...
				n.getConfiguration().clear();
				n.getConfiguration().add(configuration);
			}
	 ...
	 */
	public MitoNormalizer getNormalizer() {
		return norm;
	}
	 ...
	 */
	public void setNormalizer(MitoNormalizer norm) {
		this.norm = norm;
